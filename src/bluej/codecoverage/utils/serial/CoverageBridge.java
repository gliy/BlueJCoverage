...
public class CoverageBridge
{
    ...
    /**
     * Turn each class in the package into our representation and return it in a package.
     * <p>
     * First it converts all Jacoco's ClassCoverage objects into {@link CoverageClass},
     * then performs a merge on all of the gathered class level coverage
     * information.</br>
     * 
     * The merge is required because Jacoco returns a different ClassCoverage for every
     * class, included nested classes. </br> While Jacoco's ISourceFileCoverage returns
     * only 1 element per file, like we want, it lacks information like method locations.
     * 
     * @param pkg
    ...
    /**
     * Merges all class level coverage information, so there is 1 CoverageClass per source file.
     * <p>
     * This is done by finding the CoverageClass objects whose class name matches the name of the source file.</br>
     * Once those are filtered all remaining classes are added as children to the CoverageClass that has the same source file name.
     * @param classes the list of CoverageClasses to merge
     * @param sourceCoverage Map of package + source file name to the coverage for a source file.
     * @return reduced list of CoverageClasses, with 1 per source file.
     */
    private static List<CoverageClass> merge(List<CoverageClass> classes, Map<String, ISourceFileCoverage> sourceCoverage)
    {
        ...
        for (CoverageClass clz : classes)
        {
            ...
            if(clz.getName().equals(clz.getSourceFileName().substring(0, clz.getSourceFileName().indexOf(".java")))) {
                // use the source file coverage if there is more then 1 class in the file
     ...
     * <p>
     * By Default the class name for an inner class is ParentClass$1 for Anonymous classes,
     * and ParentClass$ChildClass for nested classes.
     * @param inner the class to find the name for
     ...
     * <p>
     * CoverageClass contains coverage information for the class, as well as
     * a list of methods, and a list of CoverageClasses, in the case of nested classes.
     * @param clz
     * @return
     */
     ...
     * @param src the Jacoco object
     * @param dest our representation of the jacoco object.
     */
